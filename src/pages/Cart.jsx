...
export default function Cart() {
  const { cartItems, count } = useContext(StylesContext);
  const totalMRP = cartItems.reduce((mrp, item) => mrp + Number(item.price), 0)
  const totalDiscount = cartItems.reduce((discount, item) => Number((discount + item.price * (item.discount / 100)).toFixed(2)),0)
  ...
  return (
    <div className={styles.cart}>
      ...
      {/* when hard refreshing the page, items in cart are getting deleted, how to prevent this? */}
      {/* Do we have to save them in database? */}
        ...
        <div className={styles.cartItems}>
          ...
          <div className={styles.priceDetails}>
            ...
            <div>
              ...
              <p className={styles.amount}>{Number((totalMRP - totalDiscount + platformFee).toFixed(2))}</p>
            </div>

            <br></br>
            <AddItemOrderButton>Place Order</AddItemOrderButton>
          </div>
        </div>
      ) : (
